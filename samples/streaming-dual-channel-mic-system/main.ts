...
// Both iframes stay mounted so audio keeps playing in whichever was started.
// Tabs only toggle which one is visible.
const ytTabBtns = document.querySelectorAll<HTMLButtonElement>(".yt-tab-btn")
const ytSlots = document.querySelectorAll<HTMLDivElement>(".yt-slot")
for (const btn of ytTabBtns) {
  btn.addEventListener("click", () => {
    const active = btn.dataset.ytTab
    for (const b of ytTabBtns) b.classList.toggle("active", b === btn)
    for (const slot of ytSlots) {
      slot.classList.toggle("hidden", slot.dataset.ytSlot !== active)
    }
  })
}
